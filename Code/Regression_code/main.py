...
for epoch in range(Epoch):
    ...
    if metric_test[0] > 0.2 and metric_test[0] > metric_test_record:
...
metric_test = test(model, data_loaders["Test_data"], device,"Test_data")
#保存指标结果
performance_result = pd.DataFrame([metric_train, metric_test], columns=[ 'Person','R^2'], index=['train_set', 'test_set'])
# 写入 Excel 文件
